...
void fillHemVertexArray(Vertex hemVertices[(HEM_LONGS + 1) * (HEM_LATS + 1)])
{
   ...
   for (j = 0; j <= HEM_LATS; j++)
      for (i = 0; i <= HEM_LONGS; i++)
      {
		 ...
		 hemVertices[k].normal.x = cos((float)j / HEM_LATS * PI / 2.0) * cos(2.0 * (float)i / HEM_LONGS * PI);
		 hemVertices[k].normal.y = sin((float)j / HEM_LATS * PI / 2.0);
		 hemVertices[k].normal.z = cos((float)j / HEM_LATS * PI / 2.0) * sin(2.0 * (float)i / HEM_LONGS * PI);
		 k++;
	  }
}
